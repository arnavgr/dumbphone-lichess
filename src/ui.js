...
export function page(title, body, session, opts = {}) {
  ...
  // #app is a plain, inert <div> on browsers with no JavaScript (dumbphones)
  // - it changes nothing about how the page renders or works there; the
  // meta-refresh tag above and ordinary <a href>/<form> navigation are still
  // exactly what drives everything. On JavaScript-capable browsers (e.g. a
  // Cloudmosa/Puffin-style "cloudphone"), public/app.js reads the data-*
  // attributes below to do faster in-place polling and AJAX navigation
  // instead of full page reloads, and to auto-size the board to the real
  // screen. Neither the CSS nor the JS below can do anything on a browser
  // that can't load/run them, so this is purely additive.
  const appAttrs = [
    'id="app"',
    opts.refreshSeconds ? `data-refresh-seconds="${opts.refreshSeconds}"` : '',
    opts.refreshUrl ? `data-refresh-url="${escapeHtml(opts.refreshUrl)}"` : '',
    opts.autosize ? 'data-autosize="1"' : '',
  ].filter(Boolean).join(' ');
  return `<!DOCTYPE html>

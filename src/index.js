...
import {
  ...
  pickAiMove, applyAiMove, BOARD_SIZE_KEYS, CUSTOM_CELL_MIN, CUSTOM_CELL_MAX,
} from './chess.js';
...
function boardSize(c) {
  ...
  const v = cookies['bsize'];
  if (BOARD_SIZE_KEYS.includes(v)) return v;
  if (v === 'custom') {
    // Set by app.js on JS-capable ("cloudphone") browsers, which measure
    // their own viewport and tell us the exact pixel cell size to use
    // instead of picking from the four fixed presets.
    const cell = parseInt(cookies['bcell'], 10);
    if (Number.isFinite(cell) && cell >= CUSTOM_CELL_MIN && cell <= CUSTOM_CELL_MAX) return cell;
  }
  return 'normal';
}
...
app.get('/settings', async (c) => {
  ...
  const requestedValid = requested && (BOARD_SIZE_KEYS.includes(requested) || requested === 'custom');
  let extraHeaders = {};
  if (requestedValid) {
    extraHeaders = { 'Set-Cookie': `bsize=${requested}; Path=/; Max-Age=31536000` };
  }
  const activeKey = requestedValid ? requested : (parseCookies(c)['bsize'] || 'normal');
  let body = '<p>Pick a board size for your screen. Saved on this phone.</p>';
  body += '<p style="font-size:12px;">Phones with JavaScript (e.g. a Cloudmosa/Puffin-style "cloudphone" browser) can also size the board automatically to fit the screen exactly - see "Automatic" below.</p>';
  ...
  body += '<hr>';
  const autoActive = activeKey === 'custom';
  const autoCell = autoActive ? boardSize(c) : null; // a number once app.js has measured the screen, else still 'normal'
  body += '<p><b>Automatic (JS-detected)</b> - a JavaScript-capable browser measures its own screen and requests the exact size, rather than one of the fixed presets above. ';
  body += autoActive
    ? `<b style="color:#006600;">[current${typeof autoCell === 'number' ? `, ${autoCell}px cells` : ' - not detected yet, reload once JavaScript has run'}]</b>`
    : `<a href="/settings?s=custom">Use automatic size</a>`;
  body += '</p>';
  if (autoActive) body += renderBoard(START_FEN, 'white', { size: autoCell });
...
<p style="font-size:12px;">Type move (e4, Nf3, O-O, or e2e4): <input type="text" name="move" size="8" maxlength="6"> <input type="submit" value="Play"></p></form>`;
  ...
  return htmlResponse(page('Play vs AI (no login)', body, s, { autosize: true }));
});
...
app.get('/game/:id', async (c) => {
  ...
  return htmlResponse(page('Game', body, s, { refreshSeconds, refreshUrl, autosize: true }));
});
...
app.get('/puzzle/:id', async (c) => {
  ...
  return htmlResponse(page('Puzzle', body, s, { autosize: true }));
});
